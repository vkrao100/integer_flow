...
DdNode* leadingTerm(DdManager* manager, DdNode* zdd,DdNode *x[18],DdNode* one)
{
   DdNode* temp =one;
   Cudd_Ref(temp);
...
while(f!=zero)
{
	...
	{		
		...
		//Cudd_Ref(f);
		//FILE* fp = fopen("rem.dot", "w");
		//add remainder for case3 insead of f1 to look at the remainder
   		//Cudd_zddDumpDot(manager, 1, &f, inputNames, outputNames, fp);
   		//fclose(fp);
...
return rem;
 ...
 DdNode* spoly(DdManager* manager, DdNode* f,DdNode* g,DdNode *x[18],DdNode* one)
 {
...
DdNode* x[12];
   ...
   x[9]=Cudd_zddChange(manager,one,9);
   Cudd_Ref(x[9]);

   x[10]=Cudd_zddChange(manager,one,10);
   Cudd_Ref(x[10]);

   x[11]=Cudd_zddChange(manager,one,11);
   Cudd_Ref(x[11]);

   DdNode* f01 = zddUnion(manager,one,x[1]);//
   Cudd_Ref(f01);
   DdNode* f0 = zddUnion(manager,f01,x[0]);//
   Cudd_Ref(f0);

   DdNode* f11 = zddUnion(manager,x[2],x[3]);//
   Cudd_Ref(f11);
   DdNode* f12 = zddUnateProduct(manager,x[2],x[3]);//
   Cudd_Ref(f12);
   DdNode* f13 = zddUnion(manager,f11,f12);//
   Cudd_Ref(f13);
   DdNode* f1 = zddUnion(manager,x[1],f13);//
   Cudd_Ref(f1);

   DdNode* f21 = zddUnateProduct(manager,x[8],x[9]);//
   Cudd_Ref(f21);
   DdNode* f2 = zddUnion(manager,x[3],f21);//
   Cudd_Ref(f2);

   DdNode* f31 = zddUnateProduct(manager,x[4],x[5]);//
   Cudd_Ref(f31);
   DdNode* f3 = zddUnion(manager,f31,x[2]);//
   Cudd_Ref(f3);

   DdNode* f41 = zddUnion(manager,one,x[6]);//
   Cudd_Ref(f41);
   DdNode* f4 = zddUnion(manager,x[4],f41);//
   Cudd_Ref(f4);

   DdNode* f51 = zddUnion(manager,one, x[7]);//
   Cudd_Ref(f51);
   DdNode* f5 = zddUnion(manager,x[5],f51);//
   Cudd_Ref(f5);

   DdNode* f61 = zddUnateProduct(manager,x[9],x[10]);//
   Cudd_Ref(f61);
   DdNode* f6 = zddUnion(manager,f61,x[6]);//
   Cudd_Ref(f6);

   DdNode* f71 = zddUnateProduct(manager,x[8],x[11]);//
   Cudd_Ref(f71);
   DdNode* f7 = zddUnion(manager,f71,x[7]);//
   Cudd_Ref(f7);
...
printf("Good till here\n");
	remen  = MultiVariate(manager, x[0],head2,x,one);//Calculating the remainder
printf("Good till here\n");
		if(remen!=NULL)
		{
			Cudd_Ref(remen);
   ...
   FILE* fp = fopen("rem.dot", "w");
//add remainder for case3 insead of f1 to look at the remainder
   Cudd_zddDumpDot(manager, 1, &remen, inputNames, outputNames, fp);
   fclose(fp); 
   Cudd_Quit(manager);
   return 0;
} 
